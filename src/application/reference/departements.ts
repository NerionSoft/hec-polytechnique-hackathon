// Départements français — INSEE Code Officiel Géographique (COG).
// Source: https://www.insee.fr/fr/information/6800675
// 96 metropolitan départements + 5 DOM. Corsica is split as 2A / 2B.
// Used as a postal-code prefix filter on the Sirene API.
...
const CODES: ReadonlySet<string> = new Set(DEPARTEMENTS.map((d) => d.code));

export function isValidDepartementCode(code: string): boolean {
  return CODES.has(code);
}

export function getDepartement(code: string): Departement | null {
