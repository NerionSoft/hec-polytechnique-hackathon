    ...
    <div className="px-8 pb-12">
      ...
      <div className="mt-3 flex items-start justify-between gap-6 pb-6">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-foreground/55 mt-1 text-[13px]">
            {countryName(lead.geo)} · {lead.sector} · Founded {lead.founded} · {lead.headcount}{" "}
            employees
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
        <section className={cn("border-foreground/[0.08] bg-surface/60 rounded-[20px] border p-6")}>
          <div className="mb-5 flex items-baseline justify-between">
            ...
            <p className="tabular font-serif text-[44px] leading-none tracking-tight">
              {total} <span className="text-foreground/45 text-[16px]">/ 100</span>
            </p>
          </div>
          ...
          <p className="border-foreground/[0.06] text-foreground/65 mt-5 border-t pt-4 text-[12.5px] leading-relaxed">
            <span className="text-foreground font-medium">Why this matters: </span>
            Matches Athena Fund III thesis on European mid-market consolidation. Comparable to
            NordicCare ApS (already IC ready) on growth profile and geographic footprint.
          </p>
        </section>
        ...
        >
          ...
          />
          <Field label="EBITDA margin" value={`${(lead.ebitdaMargin * 100).toFixed(0)}%`} />
          ...
          {lead.ownership && <Field label="Cap table" value={lead.ownership} small />}
          {lead.glassdoor && (
            <Field label="Glassdoor" value={`${lead.glassdoor.toFixed(1)} ★`} tone="good" />
          )}
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-3">
        <p className="text-foreground/85 text-[12.5px]">{row.axis}</p>
        <p
          className={cn(
      ...
      <div className="bg-foreground/[0.05] mt-1.5 h-1.5 overflow-hidden rounded-full">
        <div
          className={cn(
            "h-full rounded-full",
      ...
      <p className="text-foreground/55 mt-1 text-[11px]">{row.rationale}</p>
    </div>
  );
}
...
function Field({
  ...
  return (
    <div className="border-foreground/[0.06] flex items-baseline justify-between gap-3 border-t pt-3 first:border-0 first:pt-0">
