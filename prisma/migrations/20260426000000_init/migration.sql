 WARN  Unsupported engine: wanted: {"node":">=24.0.0"} (current: {"node":"v20.19.5","pnpm":"10.33.2"})
...
CREATE TABLE "lead" (
    ...
    "legalName" TEXT,
    "website" TEXT,
    "websiteDiscoveredAt" TIMESTAMP(3),
    "websiteDiscoverySource" TEXT,
...
-- CreateIndex
CREATE UNIQUE INDEX "agent_output_runId_agentId_documentId_key" ON "agent_output"("runId", "agentId", "documentId");
...
-- AddForeignKey
ALTER TABLE "agent_output" ADD CONSTRAINT "agent_output_documentId_fkey" FOREIGN KEY ("documentId") REFERENCES "document"("id") ON DELETE SET NULL ON UPDATE CASCADE;
...
-- ─────────────────────────────────────────────────────────────────────────
-- Manual additions Prisma can't model: HNSW vector index + tsvector column
-- ─────────────────────────────────────────────────────────────────────────
